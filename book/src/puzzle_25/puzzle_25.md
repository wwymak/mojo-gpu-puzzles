# Puzzle 25: Async Memory Operations & Copy Overlap
...
In this puzzle, you'll transform a memory-bound 1D convolution from [Puzzle 11](../puzzle_11/puzzle_11.md) into a high-performance implementation that **hides memory latency behind computation**. This isn't just an academic exercise - these patterns are fundamental to:
...
- **Shared memory programming** ([Puzzle 8](../puzzle_08/puzzle_08.md), [Puzzle 14](../puzzle_14/puzzle_14.md)) - You'll extend matmul patterns
- **Memory coalescing** ([Puzzle 19](../puzzle_19/puzzle_19.md)) - Critical for optimal async transfers
- **Tiled processing** ([Puzzle 21](../puzzle_21/puzzle_21.md)) - The foundation for this optimization
...
Building on the async copy operations introduced in [Puzzle 14's idiomatic matmul](../puzzle_14/tiled.md#solution-idiomatic-layouttensor-tiling), you'll now focus specifically on their memory optimization potential:
...
**What's different from Puzzle 14?** While Puzzle 14 used async copy for clean tile loading in matmul, this puzzle focuses specifically on **latency hiding** - structuring algorithms to overlap expensive memory operations with useful computation work.
...
**Building on [Puzzle 11](../puzzle_11/puzzle_11.md):** This puzzle revisits the 1D convolution from Puzzle 11, but now optimizes it using async copy operations to hide memory latency behind computation. Instead of simple synchronous memory access, we'll use hardware acceleration to overlap expensive DRAM transfers with useful work.
...
- Kernel size: `KERNEL_SIZE = 5` (simple 1D convolution, same as Puzzle 11)
- Data type: `DType.float32`
- Layout: `Layout.row_major(VECTOR_SIZE)` (1D row-major)

### The async copy opportunity

**Building on Puzzle 14:** You've already seen `copy_dram_to_sram_async` used for clean tile loading in matmul. Now we'll focus on its **latency hiding capabilities** - the key to high-performance memory-bound algorithms.
...
- **Dedicated copy engines**: Modern GPUs have specialized hardware that bypasses registers and enables true compute-memory overlap (as explained in [Puzzle 14](../puzzle_14/tiled.md#solution-idiomatic-layouttensor-tiling))
...
Implement 1D convolution that uses async copy operations to overlap memory transfers with computation, following patterns from Puzzle 14's matmul implementation.
...
```mojo
{{#include ../../../problems/p25/p25.mojo:async_copy_overlap_convolution}}
```
...
```mojo
{{#include ../../../solutions/p25/p25.mojo:async_copy_overlap_convolution_solution}}
```
...
```mojo
# Phase 1: Always use async copy - hardware handles bounds automatically
...
- **Tile Creation**: `input.tile[CONV_TILE_SIZE](block_idx.x)` creates a 256-element view of the input array starting at `block_idx.x * 256`. **Note**: The implementation assumes hardware/runtime handles bounds checking and zero-padding for out-of-bounds access, which may not be true in all environments.

- **Thread Layout**: `Layout.row_major(THREADS_PER_BLOCK_ASYNC, 1)` creates a 256×1 layout that matches our block organization. This is **critical** - the layout must match the physical thread arrangement for optimal coalesced memory access. When layouts mismatch, threads may access non-contiguous memory addresses, breaking coalescing and severely degrading performance.
